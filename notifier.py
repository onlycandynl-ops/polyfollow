...
def notify_trade_opened(position: Dict, signal: Dict):
    msg = (
        f"🟢 <b>TRADE OPENED</b>\n\n"
        f"📋 <b>Market:</b> {position['question'][:80]}\n"
        f"📌 <b>Side:</b> {position['side']}\n"
        f"💰 <b>Entry:</b> {position['entry_price']:.1%}\n"
        f"💵 <b>Size:</b> ${position['cost']:.2f}\n"
...
def notify_trade_closed(position: Dict):
    ...
    reason_emoji = {"STOP_LOSS": "🛑", "TAKE_PROFIT": "🎯", "MANUAL": "👋"}.get(position.get("close_reason", ""), "📋")
...
def notify_scan_complete(signals: List[Dict], opened: int, portfolio: Dict):
    signal_lines = ""
    for s in signals[:3]:
        signal_lines += f"  • {s['question'][:50]} → {s['dominant_side']} @ {s['dominant_price']:.1%} ({s['consensus_pct']:.0%} consensus)\n"
    ...
    msg = (
        f"🔍 <b>SCAN COMPLETE</b>\n\n"
        f"📊 <b>Signals found:</b> {len(signals)}\n"
        f"✅ <b>Trades opened:</b> {opened}\n\n"
        f"<b>Top signals:</b>\n{signal_lines}\n"
